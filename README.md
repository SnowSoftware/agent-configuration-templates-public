...
> [!WARNING]  
> AN INCORRECT CONFIGURATION WILL RESULT IN THE SNOW INVENTORY AGENTS NOT WORKING!  
>  
...
1. make sure you have appropriate understanding of the agent configuration and information about the environment the agent will be used in  
    1. if unsure, use the Snow Inventory Admin Console to generate the agent configuration and then export it or reach out to snow support  
...
Every Snow Inventory Agents has a default behavior depending on the platform. If you use such item in the configuration file, that default behavior would be removed. For SystemSettings the ganularity is set to the specific `<Setting>`, for everything else it is the Node above.
...
Please use the customer registry extension
